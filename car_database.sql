...
-- Schema car_database
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `car_database` DEFAULT CHARACTER SET utf8 ;
USE `car_database` ;
...
CREATE TABLE IF NOT EXISTS `car_database`.`invoices` (
  ...
  PRIMARY KEY (`auto_increment_id`, `invoice_number`),
...
CREATE TABLE IF NOT EXISTS `car_database`.`car` (
  ...
  CONSTRAINT `fk_car_customers`
    FOREIGN KEY (`customers_customer ID` , `customers_auto_increment_id`)
    REFERENCES `car_database`.`customers` (`auto_increment_id` , `customer ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
