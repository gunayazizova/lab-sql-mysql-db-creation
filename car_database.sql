...
CREATE TABLE IF NOT EXISTS `car_database`.`customers` (
  ...
  UNIQUE INDEX `idcustomer_UNIQUE` (`customer ID` ASC) VISIBLE)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `car_database`.`invoices` (
  ...
  CONSTRAINT `fk_invoices_customers1`
    FOREIGN KEY (`customers_auto_increment_id`)
    REFERENCES `car_database`.`customers` (`customer ID`),
  CONSTRAINT `fk_invoices_salespersons1`
    FOREIGN KEY (`salespersons_auto_increment_id`)
    REFERENCES `car_database`.`salespersons` (`staff_id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb3;
...
CREATE TABLE IF NOT EXISTS `car_database`.`car` (
  ...
  INDEX `fk_car_customers_idx` (`customers_customer ID` ASC, `customers_auto_increment_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_car_customers`
    FOREIGN KEY (`customers_customer ID`)
    REFERENCES `car_database`.`customers` (`customer ID`),
  CONSTRAINT `fk_car_invoices1`
    FOREIGN KEY (`invoices_auto_increment_id`)
    REFERENCES `car_database`.`invoices` (`invoice_number`),
...
CREATE TABLE IF NOT EXISTS `car_database`.`salespersons_has_customers` (
  ...
  `customers_auto_increment_id` INT NOT NULL,
  `customers_customer ID` INT NOT NULL,
  PRIMARY KEY (`salespersons_auto_increment_id`, `salespersons_staff_id`, `customers_auto_increment_id`, `customers_customer ID`),
